...
public class JwtService {
    ...
    public String generateToken(JwtDataDto jwtData, TokenPurpose purpose) {
        ...
        return Jwts.builder()
                .subject(jwtData.username())
                .issuedAt(Date.from(now))
                .expiration(Date.from(expiration))
                .claim("userId", jwtData.id().toString())
                .claim("username", jwtData.username())
                .claim("role", jwtData.role())
                .claim("purpose", purpose.name())
                .claim("temporaryPassword", jwtData.temporaryPassword())
                .signWith(key)
                .compact();
    }
    ...
    public boolean extractTemporaryPasswordFlag(String token) {
        return extractClaim(token, claims -> claims.get("temporaryPassword", Boolean.class));
    }
